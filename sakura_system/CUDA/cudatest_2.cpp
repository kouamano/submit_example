...
*/
   ...
   mk_end=omp_get_wtime();
   printf("data creation time = %.3f\n",mk_end-mk_start);
/*
...
void mm_gpu(const double *A,const double *B,double *C,int hA,int wA,int wB,int tile,int num_of_devices)
{
 ...
 printf("tile = %d device(s) = %d wA = %d hA = %d wB = %d\n",tile,num_of_devices,wA,hA,wB);
 for(i=0;i<num_of_devices;i++)
   devices[i]=i;

 cublasXtCreate(&handle);
 cublasXtDeviceSelect(handle, num_of_devices, devices);
 cublasXtSetBlockDim(handle, tile);
