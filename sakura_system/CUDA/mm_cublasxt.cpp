...
extern void mm_gpu(const double *A,const double *B,double *C,int hA,int wA,int wB);
...
int main(int argc,char *argv[])
{
   ...
   mm_gpu(A,B,GC,hA,wA,wB);
   gpu_end=omp_get_wtime();
   gpu_wtime=gpu_end-gpu_start;
   check_result(CC,GC,hA,wA,wB,1e-05);
...
void mm_gpu(const double *A,const double *B,double *C,int hA,int wA,int wB)
{
